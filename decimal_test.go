...
func TestParse(t *testing.T) {
	...
	assert := func(strV string, value int64, precission int) {
		...
		if dec.value != value {
			t.Errorf("expected value %d but was %d ", value, dec.value)
		}
		if dec.precision != precission {
			t.Errorf("expected value %d but was %d ", precission, dec.precision)
		}
	}
	assert("1", 1, 0)
	assert("1,0", 10, 1)
...
func TestPrint(t *testing.T) {
	...
	if a_1234_2.Format() != "12,34 Eur" {
		t.Errorf("Falsches Format. want '12,34 Eur' got '%v'", a_1234_2.Format())
	}
}
...
func TestFormatPrecission(t *testing.T) {
	...
	assert(Decimal{value: 12345678912, precision: 4}, 2, "1234567,89 Eur")
	assert(Decimal{value: 10199, precision: 2}, 2, "101,99 Eur")
	assert(Decimal{value: -10199, precision: 2}, 2, "-101,99 Eur")
}
